package com.dx.dxintercambio;

public class Post6 {

    private String user ;

    private String password ;

    private int idRemolque ;
    private String idSQL ;
    private int idIntercambio ;
    private String sello1 ;
    private String sello2 ;
    private String llanta1 ;
    private String llanta2 ;
    private String llanta3 ;
    private String llanta4 ;
    private String llanta5;
    private String llanta6 ;
    private String llanta7 ;
    private String llanta8 ;
    private int llantajumbo ;
    private int llantajumbo2 ;
    private int selloExtra ;
    private String sello3 ;
    private int incidencia ;
    private String incidenciaCancel ;

    private String defensa ;
    private String motor ;
    private String piso ;
    private String tanqueDeComb ;
    private String llantas ;
    private String diferencial ;
    private String cabina ;
    private String cilindrosDeAire ;
    private String mofleEscape ;
    private String quintaRueda ;
    private String remolque ;
    private String chasis ;
    private String puertasTraseras ;
    private String  paredes ;
    private String sellos ;
    private String lucesLatAmbar ;
    private String lucesDeFrente ;
    private String cuartosAmbar ;
    private String lucesTraseras ;
    private String cuartosRojos ;
    private String lucesDeAltoTraseros ;
    private String luzDePlaca ;
    private String zoqueteras ;
    private String manivela ;
    private String guardaPolvos;
    private String loderas ;
    private String llantaDeRefaccion ;
    private String placas ;
    private String  placasDatos ;
    private String comentario2;

    public Post6(String user, String password, int idRemolque, String idSQL, int idIntercambio, String sello1, String sello2, String llanta1, String llanta2, String llanta3, String llanta4, String llanta5, String llanta6, String llanta7, String llanta8, int llantajumbo, int llantajumbo2, int selloExtra, String sello3, int incidencia, String incidenciaCancel, String defensa, String motor, String piso, String tanqueDeComb, String llantas, String diferencial, String cabina, String cilindrosDeAire, String mofleEscape, String quintaRueda, String remolque, String chasis, String puertasTraseras, String paredes, String sellos, String lucesLatAmbar, String lucesDeFrente, String cuartosAmbar, String lucesTraseras, String cuartosRojos, String lucesDeAltoTraseros, String luzDePlaca, String zoqueteras, String manivela, String guardaPolvos, String loderas, String llantaDeRefaccion, String placas, String placasDatos, String comentario2) {
        this.user = user;
        this.password = password;
        this.idRemolque = idRemolque;
        this.idSQL = idSQL;
        this.idIntercambio = idIntercambio;
        this.sello1 = sello1;
        this.sello2 = sello2;
        this.llanta1 = llanta1;
        this.llanta2 = llanta2;
        this.llanta3 = llanta3;
        this.llanta4 = llanta4;
        this.llanta5 = llanta5;
        this.llanta6 = llanta6;
        this.llanta7 = llanta7;
        this.llanta8 = llanta8;
        this.llantajumbo = llantajumbo;
        this.llantajumbo2 = llantajumbo2;
        this.selloExtra = selloExtra;
        this.sello3 = sello3;
        this.incidencia = incidencia;
        this.incidenciaCancel = incidenciaCancel;
        this.defensa = defensa;
        this.motor = motor;
        this.piso = piso;
        this.tanqueDeComb = tanqueDeComb;
        this.llantas = llantas;
        this.diferencial = diferencial;
        this.cabina = cabina;
        this.cilindrosDeAire = cilindrosDeAire;
        this.mofleEscape = mofleEscape;
        this.quintaRueda = quintaRueda;
        this.remolque = remolque;
        this.chasis = chasis;
        this.puertasTraseras = puertasTraseras;
        this.paredes = paredes;
        this.sellos = sellos;
        this.lucesLatAmbar = lucesLatAmbar;
        this.lucesDeFrente = lucesDeFrente;
        this.cuartosAmbar = cuartosAmbar;
        this.lucesTraseras = lucesTraseras;
        this.cuartosRojos = cuartosRojos;
        this.lucesDeAltoTraseros = lucesDeAltoTraseros;
        this.luzDePlaca = luzDePlaca;
        this.zoqueteras = zoqueteras;
        this.manivela = manivela;
        this.guardaPolvos = guardaPolvos;
        this.loderas = loderas;
        this.llantaDeRefaccion = llantaDeRefaccion;
        this.placas = placas;
        this.placasDatos = placasDatos;
        this.comentario2 = comentario2;
    }

    public String getUser() {
        return user;
    }

    public String getPassword() {
        return password;
    }

    public int getIdRemolque() {
        return idRemolque;
    }

    public String getIdSQL() {
        return idSQL;
    }

    public int getIdIntercambio() {
        return idIntercambio;
    }

    public String getSello1() {
        return sello1;
    }

    public String getSello2() {
        return sello2;
    }

    public String getLlanta1() {
        return llanta1;
    }

    public String getLlanta2() {
        return llanta2;
    }

    public String getLlanta3() {
        return llanta3;
    }

    public String getLlanta4() {
        return llanta4;
    }

    public String getLlanta5() {
        return llanta5;
    }

    public String getLlanta6() {
        return llanta6;
    }

    public String getLlanta7() {
        return llanta7;
    }

    public String getLlanta8() {
        return llanta8;
    }

    public int getLlantajumbo() {
        return llantajumbo;
    }

    public int getLlantajumbo2() {
        return llantajumbo2;
    }

    public int getSelloExtra() {
        return selloExtra;
    }

    public String getSello3() {
        return sello3;
    }

    public int getIncidencia() {
        return incidencia;
    }

    public String getIncidenciaCancel() {
        return incidenciaCancel;
    }

    public String getDefensa() {
        return defensa;
    }

    public String getMotor() {
        return motor;
    }

    public String getPiso() {
        return piso;
    }

    public String getTanqueDeComb() {
        return tanqueDeComb;
    }

    public String getLlantas() {
        return llantas;
    }

    public String getDiferencial() {
        return diferencial;
    }

    public String getCabina() {
        return cabina;
    }

    public String getCilindrosDeAire() {
        return cilindrosDeAire;
    }

    public String getMofleEscape() {
        return mofleEscape;
    }

    public String getQuintaRueda() {
        return quintaRueda;
    }

    public String getRemolque() {
        return remolque;
    }

    public String getChasis() {
        return chasis;
    }

    public String getPuertasTraseras() {
        return puertasTraseras;
    }

    public String getParedes() {
        return paredes;
    }

    public String getSellos() {
        return sellos;
    }

    public String getLucesLatAmbar() {
        return lucesLatAmbar;
    }

    public String getLucesDeFrente() {
        return lucesDeFrente;
    }

    public String getCuartosAmbar() {
        return cuartosAmbar;
    }

    public String getLucesTraseras() {
        return lucesTraseras;
    }

    public String getCuartosRojos() {
        return cuartosRojos;
    }

    public String getLucesDeAltoTraseros() {
        return lucesDeAltoTraseros;
    }

    public String getLuzDePlaca() {
        return luzDePlaca;
    }

    public String getZoqueteras() {
        return zoqueteras;
    }

    public String getManivela() {
        return manivela;
    }

    public String getGuardaPolvos() {
        return guardaPolvos;
    }

    public String getLoderas() {
        return loderas;
    }

    public String getLlantaDeRefaccion() {
        return llantaDeRefaccion;
    }

    public String getPlacas() {
        return placas;
    }

    public String getPlacasDatos() {
        return placasDatos;
    }

    public String getComentario2() {
        return comentario2;
    }
}
